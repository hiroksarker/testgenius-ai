...
export class TestRecorder {
  ...
  private async setupDirectoryStructure(): Promise<void> {
    ...
    for (const dir of directories) {
      try {
        ...
        console.log(chalk.yellow(`⚠️  Could not create ${dir}/ directory, trying next...`));
        continue;
      }
    }
    
    if (!createdDir) {
      console.log(chalk.red('❌ Could not create any test directory. Please create a tests directory manually.'));
      throw new Error('No test directory could be created');
    }
    
    // Store the created directory for later use
    this.testDirectory = createdDir;
    console.log(chalk.green(`📁 Tests will be saved to ${createdDir}/ directory\n`));
  }
  ...
  private async getTestInfo(): Promise<void> {
    ...
    const answers = await inquirer.prompt([
      ...
      },
      {
        ...
        type: 'list',
        choices: [
          { name: '🚀 Enhanced Async Format (Recommended)', value: 'enhanced' },
          { name: '📝 Legacy Format', value: 'legacy' }
        ],
        default: 'enhanced'
      }
    ]);
  ...
  private async finishRecording(save: boolean): Promise<void> {
    ...
    if (!this.testDirectory) {
      console.log(chalk.red('❌ No test directory available. Please restart the recorder.'));
      return;
    }
    
    const filePath = path.join(process.cwd(), this.testDirectory, fileName);
    ...
    try {
      ...
      console.log(chalk.blue(`📁 File: ${this.testDirectory}/${fileName}`));
      ...
      console.log(chalk.red(`❌ Could not save test file: ${(error as Error).message}`));
      console.log(chalk.yellow('Please check file permissions and try again.'));
    }
  }

  private generateFileContent(test: TestDefinition): string {
    const testId = this.testInfo.id?.replace(/[^a-zA-Z0-9]/g, '_').toUpperCase();
    const format = (this.testInfo as any).format || 'enhanced';
